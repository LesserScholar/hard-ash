...
optimizers = {
    'rmsprop' : {
        ...
        'learning_rate': Vs([4e-6, 5e-6, 5.5e-6, 6e-6, 8e-6], manual_value=9e-6), #logU(4e-6, 6e-6),
    },
    'adam' : {
        'adam_b1': Vs([0.9, 0.95, 0.98, 0.99], manual_value=0.9),
        'adam_b2': Vs([0.999, 0.9995], manual_value=0.999),
        'learning_rate': Vs([8e-6, 1e-5, 1.5e-5], manual_value=1e-3), #logU(4e-6, 6e-6),
    },
    'adagrad' : {
...
if sys.argv[2].startswith('all'):
    ...
    if len(split) > 1:
        optimizer_skip = int(split[1])
        optimizers_to_run = list(optimizers_to_run)[optimizer_skip:]
else:
...
for chosen_optimizer_config in optimizers_to_run:
    ...
    if is_sweep:
        sweep_id = wandb.sweep(sweep_config, project='mnist paper iid', entity="skeskinen")
